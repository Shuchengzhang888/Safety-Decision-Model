...
class Vehicle(RoadObject):
    ...
    def __init__(self,
        ...
        self.pred_position3 = np.array(position, dtype=np.float64)
    ...
    def step(self, dt: float) -> None:
        ...
        pred_v1 = self.speed * np.array([np.cos(self.heading + beta),
                                        np.sin(self.heading + beta)])
        self.pred_position1 = self.position + pred_v1*dt*5
        pred_heading1 = self.heading + self.speed * np.sin(beta) / (self.LENGTH / 2) * dt*5
        pred_speed1 = self.speed + self.action['acceleration'] * dt*5
        #pred2
        pred_v2 = pred_speed1 * np.array([np.cos(pred_heading1 + beta),
                                        np.sin(pred_heading1 + beta)])
        self.pred_position2 = self.pred_position1 + pred_v2*dt*5
        pred_heading2 = pred_heading1 + pred_speed1 * np.sin(beta) / (self.LENGTH / 2) * dt*5
        pred_speed2 = pred_speed1 + self.action['acceleration'] * dt*5
        #pred2
        pred_v3 = pred_speed2 * np.array([np.cos(pred_heading2 + beta),
                                        np.sin(pred_heading2 + beta)])
        self.pred_position3 = self.pred_position2 + pred_v3*dt*5
        pred_heading3 = pred_heading2 + pred_speed2 * np.sin(beta) / (self.LENGTH / 2) * dt*5
        pred_speed3 = pred_speed2 + self.action['acceleration'] * dt*5
